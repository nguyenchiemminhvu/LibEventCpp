...
/**
 * Demonstrate notifier stop/resume lifecycle
 */
void demonstrate_notifier_stop_resume(const std::string& path)
{
    try
    {
        if (!ensure_directory_exists(path))
        ...
        if (wd < 0)
        {
            std::cerr << "Error: Failed to add watch: " << wm->get_last_error() << std::endl;
            return;
        }

        auto notifier = std::make_shared<fs_event::notifier>(wm, handler);

        // Phase 1: run the event loop in a background thread
        std::cout << "[Phase 1] Notifier running for 3 seconds. Create/modify files in: " << path << std::endl;
        std::thread loop_thread([&notifier]() { notifier->loop(); });
        std::this_thread::sleep_for(std::chrono::seconds(3));

        std::cout << "Stopping notifier..." << std::endl;
        notifier->stop();
        loop_thread.join();
        std::cout << "Notifier stopped. Events will not be processed for 2 seconds." << std::endl;
        std::this_thread::sleep_for(std::chrono::seconds(2));

        // Phase 2: resume and run again
        std::cout << "\nResuming notifier..." << std::endl;
        notifier->resume();
        loop_thread = std::thread([&notifier]() { notifier->loop(); });
        std::cout << "[Phase 2] Notifier running again for 3 more seconds." << std::endl;
        std::this_thread::sleep_for(std::chrono::seconds(3));

        notifier->stop();
        loop_thread.join();
        std::cout << "Notifier stopped again. Stop/resume demo complete." << std::endl;
    }
    catch (const std::exception& e)
    {
        std::cerr << "Exception: " << e.what() << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    std::cout << "4. Stop/Resume notifier demo" << std::endl;
    ...
    switch (choice)
    {
        ...
        case 3:
            ...
            break;
        case 4:
            demonstrate_notifier_stop_resume(path_to_watch);
            break;
        case 1:
        default:
