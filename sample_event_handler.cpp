...
int main()
{
    ...
    // Demo: stop/resume lifecycle
    std::cout << "\n--- Stop/Resume demo for event_handler ---" << std::endl;
    std::cout << "Handler is stopped. Events posted now will be discarded." << std::endl;
    handler->post_event(&TestHandler::handle_a); // discarded

    std::this_thread::sleep_for(std::chrono::milliseconds(200));
    std::cout << "Resuming handler..." << std::endl;
    handler->resume();
    handler->post_event(&TestHandler::handle_a);
    handler->post_event(&TestHandler::handle_b);
    std::this_thread::sleep_for(std::chrono::seconds(2));
    std::cout << "Stopping handler again." << std::endl;
    handler->stop();
