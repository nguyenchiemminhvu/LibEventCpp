...
void demonstrate_once_at_least()
{
    ...
    data_stream.stop();
    std::this_thread::sleep_for(std::chrono::milliseconds(2000)); // Wait to ensure no more processing occurs

    // Demo: resume once_at_least after stop
    std::cout << "\n--- Resuming once_at_least ---" << std::endl;
    data_stream.resume();
    std::this_thread::sleep_for(std::chrono::milliseconds(3500));
    std::cout << "Stopping once_at_least again." << std::endl;
    data_stream.stop();
}
